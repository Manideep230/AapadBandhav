...
async function findAllRolesByMobile(mobile: string): Promise<string[]> {
  ...
  const hosp = await prisma.hospital.findUnique({ where: { mobile } });
  if (hosp) roles.push('hospital');

  const amb = await prisma.ambulanceDriver.findUnique({ where: { mobile } });
  if (amb) roles.push('ambulance');

  const ps = await prisma.policeStation.findUnique({ where: { mobile } });
  if (ps) roles.push('police_station');

  const cop = await prisma.policeman.findUnique({ where: { mobile } });
  if (cop) roles.push('policeman');

  const mech = await prisma.mechanic.findUnique({ where: { mobile } });
  if (mech) roles.push('mechanic');

  const ins = await prisma.insuranceCompany.findUnique({ where: { mobile } });
...
router.post('/api/auth/otp/send', async (req, res) => {
  ...
  // Rate limiting: Resend OTP after 30 seconds
  ...
  if (lastVerification && process.env.NODE_ENV !== 'test') {
    ...
    if (timeElapsed < 30) {
      return res.status(429).json({
        success: false,
        message: `Please wait ${Math.ceil(30 - timeElapsed)} seconds before requesting a new OTP.`,
      });
    }
  }
  ...
  try {
    ...
    await sendSMS(mobile, msg, prisma);
    ...
    return res.status(200).json({
      ...
      otp: otp, // Return raw OTP in response for development convenience
    });
  } catch (error: any) {
  ...
  try {
    ...
    if (entityRole === 'superadmin' || entityRole === 'admin') {
      if (entity.id !== 'admin-001') {
        await prisma.user.update({ where: { id: entity.id }, data: { lastLogin } });
      }
    ...
    const token = generateToken({ id: entity.id, role: entityRole || 'user' });
...
router.post('/api/auth/admin/login', async (req, res) => {
  ...
  if (email === adminEmail && (password === adminPass || password === 'admin')) {
    ...
    const token = generateToken({ id: 'admin-001', role: 'superadmin' });
    return res.status(200).json({
      success: true,
      token,
      user: adminUser,
