...
import { processGpsSpeedAndLogs, checkAndUpdateDeviceStops } from './utils/gps';
    ...
    if (parts.length === 3 && parts[0] === 'vehicle') {
      ...
      if (device) {
        ...
        if (lat !== null && lng !== null) {
          speedVal = await processGpsSpeedAndLogs(prisma, device.deviceId, lat, lng, now);
          await checkAndUpdateDeviceStops(prisma, device.deviceId, lat, lng, speedVal);
        }
