...
import { triggerRealtimeEvent } from './pusher';
...
export function estimateETA(distanceKm: number, avgSpeedKmh: number = 40): number {
  return Math.round((distanceKm / avgSpeedKmh) * 60);
}

export async function processGpsSpeedAndLogs(
  prisma: PrismaClient,
  deviceCode: string,
  lat: number,
  lng: number,
  ...
  let speed = 0.0;
  if (lastLog) {
    const dist = haversineDistance(lastLog.latitude, lastLog.longitude, lat, lng);
...
export async function checkAndUpdateDeviceStops(
  prisma: PrismaClient,
  deviceCode: string,
  lat: number,
  lng: number,
    ...
    if (isStopped) {
      if (!activeSeg) {
        ...
        if (prevSeg) {
          ...
          // Emit Pusher events
          await triggerRealtimeEvent(`device-${device.deviceId}`, 'movement', payload);
          if (device.ownerId) {
            await triggerRealtimeEvent(`user-${device.ownerId}`, 'device-movement', payload);
          }
        }
      }
    } else {
