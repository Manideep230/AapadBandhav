  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      while (true) {
        ...
        const dup = await prisma.device.findUnique({ where: { deviceId: devCode } });
        if (!dup) break;
      }
...
router.get('/api/admin/devices/assigned', withAuth(async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    for (const d of devices) {
      const user = await prisma.user.findUnique({ where: { id: d.ownerId || '' } });
...
async function isMobileReserved(mobile: string): Promise<boolean> {
  ...
  if (await prisma.user.findUnique({ where: { mobile } })) return true;
  if (await prisma.hospital.findUnique({ where: { mobile } })) return true;
  if (await prisma.ambulanceDriver.findUnique({ where: { mobile } })) return true;
  if (await prisma.policeStation.findUnique({ where: { mobile } })) return true;
  if (await prisma.policeman.findUnique({ where: { mobile } })) return true;
  if (await prisma.mechanic.findUnique({ where: { mobile } })) return true;
  if (await prisma.insuranceCompany.findUnique({ where: { mobile } })) return true;
